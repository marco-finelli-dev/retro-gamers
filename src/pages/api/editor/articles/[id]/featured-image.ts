...
import {
  ...
  updateEditorialArticleFeaturedImage,
...
export const POST: APIRoute = async ({ request, params, cookies }) => {
  ...
  try {
    const result = await updateEditorialArticleFeaturedImage({
    ...
    logApiError('editorial-articles.featured-image-api', error);

    return json({ ok: false, error: 'featured_image_failed' }, 500);
  }
};
